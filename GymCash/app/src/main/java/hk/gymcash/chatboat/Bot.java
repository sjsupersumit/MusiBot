...
public class Bot {
	...
	private Keyword parse(String text, ArrayList<Keyword> keylist) {
		...
		// add best answers regex variable value into the dictionary for future
		// reference
		if (match != null) {
			if (match.learn.length() > 0) {
				...
				messages.add(result);
				State myState = new State(String.valueOf(parser.stateCounter),
						messages, new ArrayList());
				parser.addState(myState);

				// add the new trained keyword
				Keyword keyword = new Keyword(subject, myState.getId(), "", "",
						"", 1, "");
